...
export default function App() {
  ...
  const [counts, setCounts] = useState({ pending: 0, urgent: 0, ignored: 0 });

  async function refreshCounts() {
    try {
      const [pending, urgent, ignored] = await Promise.all([
        api.previews('pending'), api.previews('pending'), api.previews('ignored'),
      ]);
      setCounts({
